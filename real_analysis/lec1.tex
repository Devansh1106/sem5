...
\begin{ax}[Axiom of Choice]
    For any collection of non-empty sets $C = \{A_i \mid i \in I\}$ where $I$ is some indexing set, there exists a function called counting function $f : C \to \cup_i A_i$. This function maps every $A_i$ to some element of $A_i$.
\end{ax}
Example of theorem that needs AOC:
\begin{thm}
    Countable union of the countable sets is countable.
\end{thm}
\begin{proof}
    Let $C = \{S_n \mid n \in \N\}$. Each element of $C$ is countable hence there exists a surjection $f_j : \N \to S_j$. Consider $\cup_i S_i$ be the countable union of $S_i$. Define $k : \N \times \N \to \cup_i S_i$ such that $k(m,n) = f_m(n)$. For each $x \in \cup_i S_i \implies x \in S_j$ for some $j \in \N$. And there exists $n \in \N$ such that $f_j(n) = x$. Therefore $k(j,n) = f_j(n)$ and $k$ is a surjection. By lemma below, $\cup_iS_i$ is countable.
\end{proof}
\begin{lem}
    The following statements are equivalent:
    \begin{enumerate}
        \item $S$ is countable.
        \item $\exists$ a surjection from a subset of $\N \to S$.
        \item $\exists$ an injection from $S \to \N.$
    \end{enumerate}
\end{lem}
\begin{proof}
    ($1 \implies 2$) Since we can count each element of $S$ hence it is a surjection.\\
    ($2 \implies 3$) For each element $s$ of $S$ there exists atleast a $n \in \N$ such that $f(n) = s$. Take $\min\{n \in \N : f(n) = s\}$ and map $s$ to that minimum element. This will result in an injection.\\
    ($3 \implies 1$) If there exists an injection from $S \to \N$ then there exists a surjection from a subset of $\N$ to $S$. Hence, $S$ is countable.
\end{proof}
\begin{rem}[\bf Where is AOC is used?]
    Notice that there exists many surjection $f_j : \N \to S_j$ for each $j$. Let $C = \{E_j : j \in \N\}$ where each $E_j$ is a collection of surjections from $\N \to S_j$ for each $j$. Hence, we need to choose an element (surjection) for each element for $C$. This is allowed because of AOC.
\end{rem}
\begin{thm}
    If $f : A \to B$ is a surjection, then $B$ is injective to a subset of $A$. 
\end{thm}
\begin{proof}
    For each $b \in B$ there exists atleast $a \in A$ such that $f(a) = b$. Take $\min\{a \in A : f(a) = b\}$ or one of those $a$ for each $b \in B$. This defines the injection between $A$ and $B$.
\end{proof}
\begin{rem}
    AOC is used when we make a choice of one of the $a \in A$ since there can be many possible $a$-s.
\end{rem}
\begin{defn}[Partial ordering relation on a set $P$]
    A partial order on a set is a relation $\leq$ defined on a set that follows
    \begin{enumerate}
        \item $\forall x \in P, x \leq x$ (reflexive).
        \item If $x, y \in P$ and $x \leq y$ and $y \leq x$ then $x = y$ (antisymmetry).
        \item $\forall x, y, z \in P$, if $x \leq y$ and $y \leq z$ then $x \leq z$. 
    \end{enumerate}
    Any set $P$ which is partially ordered is called {\bf poset}. A set $C$ which is totally ordered and is a subset of $P$ is called a {\bf chain} of $P$.
\end{defn}
\begin{defn}[Total order on a set $C$]
    A relation that is a partial order with additional condition that for $x,y \in C$, either $x \leq y$ or $y \leq x$ is true (comparability). Then set $C$ is {\it totally} ordered.
\end{defn}
\begin{rem}
    Totally order relation is $\leq$ and strict totally order relation is $<$. Depending upon the definition of order relation (defined for strict or non-strict), we can say total order is an ordering relation (in case of non-strict) or strict totally order is an order relation (in case of strict).
\end{rem}
\begin{eg*}
\begin{enumerate}
    \item Order relation on $\R$ is totally ordered due to trichotomy properties.
    \item If the ordering relation is $a \in \Z \mid b \in \Z$ then it is a partial order (reflexive, antisymmetric, transitive). But it fails to be totally ordered ($3$ and $5$ are not related).
    \item For the set $1,p ,p^2, \dots$ where $p$ is prime, we can see this is totally ordered with respect to above order relation. ({\bf Remark.} $1, p, p^2, \dots$ is a chain of poset $\Z$. Why prime? Because composite numbers and their multiples will be covered in multiples of primes.)
\end{enumerate}
\end{eg*}
\begin{defn}[Boundedness of a subset $C$ of poset $P$]
    We say subset $C$ in a poset $P$ is bounded, or has an upper bound, if $\exists M \in P$ so that $\forall q \in C, q \leq M$.   
\end{defn}
\begin{ax}[Zorn's lemma]
    Given a poset $P$, if for every chain $C$ of $P$, there exists an upper bound $M_c$ in $P$, then $P$ has a maximal element with respect to $\leq$ (i.e. $\exists M \in P$ such that $\forall x \in P, x \leq M$.)
\end{ax}
\begin{thm}[Cantor's theorem]
    For any set $A$, there does not exist any surjection from $A$ to $P(A)$.
\end{thm}
\begin{proof}
    Suppose for constradiction that there exists a surjection $f: A \to P(A)$. This means that for each $B \subset P(A)$, there exists $a \in A$ such that $f(a) = B$. Also, $P(A)$ is injective to some subset $A'$ of $A$ (we can construct this by taking minimum of all $a$'s for which $f(a) = B$). Now, $f$ is surjection as well as injection hence $|A'| = |P(A)|$ which is a contradiction. Therefore, there does not exists any surjection from $A \to P(A)$.
\end{proof}

\subsection*{The Real and Complex fields}
\begin{ax}[Completeness axiom for the Real Number Field]
    Every non empty subset of $\R$ that is bounded above has a supremum.
\end{ax}
\begin{cor}
    Every non empty subset of $\R$ that is bounded below has an infimum.
\end{cor}
\begin{proof}
